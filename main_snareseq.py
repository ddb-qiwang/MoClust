...
def get_args():
    ...
    parser.add_argument('--ATAC_raw_matrix', type=str,default='/data/msyuan/scMVC/data/10x10k_rawdata/atac_mat.csv', help='The path of ATAC raw matrix')
    parser.add_argument('--gtf', type=str,default='/data/msyuan/Datasets/multiomics_datasets/homo_sapiens/gencode.v39.annotation.gtf', help='The path of gtf file')
    ...
    parser.add_argument('--encoder_atac_layer', type=list, default=[256,32])
...
def main():
    ...
    rna_mat = pd.read_csv(args.RNA_raw_matrix,index_col=0)
    atac_mat = np.array(pd.read_csv(args.ATAC_raw_matrix,index_col=0))
    atac = sc.AnnData(np.array(atac_mat))
    atac.var_names = atac_mat.columns
    gaX = geneactivity(atac, mousegtf, maohao=True)
    ga_mat = ga.X
    if args.have_labels:
        ...
        #build dataset
        views = [rna_mat, ga_mat]
        train_dataset = multiviewDataset(views, labels, device, highly_genes=[[0,1],[args.highly_genes,args.highly_genes]])
        ...
        encoder2_cfg = encoder_cfg(Layer=tuple([np.shape((train_dataset.views[1].X))[1]]+args.encoder_atac_layer))
        ...
        views = [rna_mat, ga_mat]
        train_dataset = multiviewDataset_nolabel(views, device, highly_genes=[[0,1],[args.highly_genes,args.highly_genes]])
        sampler = torch.utils.data.RandomSampler(train_dataset)
        train_loader = torch.utils.data.DataLoader(
            train_dataset,
            sampler=sampler,
